...
class RestScheduleService:
    ...
    def add_window(
        ...
        # Merge overlapping windows to keep a single continuous block.
    ...
    def is_resting(self, chat_id: int, when: Optional[datetime] = None) -> bool:
        return self.current_window(chat_id, when) is not None

    def current_window(self, chat_id: int, when: Optional[datetime] = None) -> Optional[RestWindow]:
        self._prune_expired()
    ...
    def next_resume_time(self, chat_id: int, when: Optional[datetime] = None) -> Optional[datetime]:
        ...
        future_ends = [
            ...
            for payload in self._data.values()
            if payload["chat_id"] == chat_id and payload.get("status") == "approved" and _parse_dt(payload["end"]) > when
        ]
        return min(future_ends) if future_ends else None

    def next_window(self, chat_id: int) -> Optional[RestWindow]:
        windows = self.list_windows(chat_id, include_past=False)
