...
- **统一时区**：Agent 面向用户展示的所有时间（日志、提醒、`/rest`、`/next` 等）均转换为北京时间（UTC+8），与代码中的 `core/utils/timezone.py` 保持一致，避免“本地时间/服务器时间”混淆。
- **休息与勿扰**：通过 `/rest` 或 LLM 的 `rest_*` 工具创建的窗口会暂停主动提醒、追问和跟踪计时，恢复后自动顺延。
